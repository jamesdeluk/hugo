...
After [playing around](https://www.jamesgibbins.com/articles/operational-technology/openplc-arduino/) with ladder logic (LL) in OpenPLC, I wanted to get a basic grip of structured text (ST). While LL is a visual method of programming PLCs, ST is a C-like language for programming PLCs, featuring well-know coding functions such as IF and WHILE.

Fortunately, OpenPLC also allows you to program PLCs using ST! There is one minor difference; with Ladder Logic, OpenPLC can simulate the circuit and get a pretty visualisation based on the LL design itself - see the images in my [previous post](https://www.jamesgibbins.com/articles/operational-technology/openplc-arduino/#one-button-latching-circuit-with-emergency-stop). With ST, because there is no design, there is no pretty visualisation. However, the Debugger does give a timeline of the states of the various variables, and it can be used to force a value, so you can still simulate a circuit.
...
Let's recreate the [basic designs](https://www.jamesgibbins.com/articles/operational-technology/openplc-arduino/#project-examples) I created with LL before in OpenPLC and get them running on the Arduino.

The physical Arduino circuit is the same as before ([see here](https://www.jamesgibbins.com/articles/operational-technology/openplc-arduino/#project-examples)), and of course OpenPLC is installed and configured in the same way also ([see here](https://www.jamesgibbins.com/articles/operational-technology/openplc-arduino/#installation)).
...
The variables are the same as LL (I've explained the strange numbers [here](https://www.jamesgibbins.com/articles/operational-technology/openplc-arduino/#steadyish-state-eg-temperature)), except the temperatures are also included up here, and I've used initial values to make the simulation look better:
